...
def validar_Cantidad(Cantidad):
    ...
    if not isinstance(Cantidad, int) or Cantidad < 0:
        errores.append("La cantidad debe ser un número entero no negativo.")
    ...
    if not re.match("^[0-9]+$", str(Cantidad)):
...
def validar_valor_impuesto(valor):
    try:
        ...
        if not isinstance(valor, float):
            raise ValidationError('El valor del impuesto debe ser un número decimal.')
...
def validar_cai(cai):
    if not re.match(r'^(?=.*[A-Z])(?=.*\d)[A-Z\d]{32}$', cai):
        raise ValidationError('El CAI debe tener 32 caracteres y contener al menos una letra mayúscula y un número.')
...
def validar_descuento(value):
    ...
    if value <= 0 or value >= 1:
        raise ValidationError('El valor del descuento debe ser mayor que 0 y menor que 1.')
